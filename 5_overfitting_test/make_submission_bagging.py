...
model_path = [x.strip() for x in  sys.argv[1].split(',')]
out_path = sys.argv[2]
...
result = []
for j in xrange(len(model_path)):
    try:
        print "loading model " + str(model_path[j])
        model = serial.load(model_path[j])
    except Exception, e:
        usage()
